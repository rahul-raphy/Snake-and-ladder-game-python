...
def turn(score,l,s):
    ...
    if score<=100:
        ...
        while pygame.time.get_ticks()-time<1500:
            message_display1("Can't move!",650,50,35,yellow)
            pygame.display.update()
...
def play(b):
    ...
    while True:
        ...
        if l:
            time=pygame.time.get_ticks()
            while pygame.time.get_ticks()-time<2000:
                message_display1("GREAT! THERE'S A LADDER!",650,50,35,black)
                pygame.display.update()
        if s:
            time=pygame.time.get_ticks()
            while pygame.time.get_ticks()-time<2000:
                message_display1("OH NO! THERE'S A SNAKE!",650,50,35,black)
                pygame.display.update()
